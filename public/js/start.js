	...
	if (hasLowerCase(usercode)) {
		validate(usercode).then((data) => {
			if (data.userExists) window.location.href = data.redirect;
				...
				modalTrigger('Hello there!', "You know, this code actually doesn't exist, Let's try again!", 'Ok');
			}
		});
	} else {
		modalTrigger('Hello there!', "That's seems a invalid code, can you reenter a correct code", 'Yes I do');
	}
});
	...
	if (code != null && code != '') {
		validate(code).then((data) => {
			if (data.userExists) {
				...
				avatarGen(element[0], data.avatarCode, code);
				avatarGen(element[1], data.avatarCode, code);
			} else {
